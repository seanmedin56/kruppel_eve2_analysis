function [trace] = gillespie_fatal_krup(elong_time, time_res, points_per_trace, ...
...
% kon_krup: flow of kruppel into being bound
% coop_coeff: how much kruppel must be bound at once to turn transcription
% off
...
while (t < t_max)
    ...
    % checks if fatal off has occurred
    if naive_states(end) == 3
        break
    end
    
    %creates transition matrix
    kr_flow = kon_krup * cur_krup^coop_coeff;
    trans_mat = [-(gen_ks(2) + kr_flow), gen_ks(1), 0;
                 gen_ks(2), -(gen_ks(1) + kr_flow), 0;
                 kr_flow, kr_flow, 0];
    ...
    if dt >= time_to_krup_change
        ...
        naive_states = [naive_states randsample(1:3, 1, true, probs)];
    end
