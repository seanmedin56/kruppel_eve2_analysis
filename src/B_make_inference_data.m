...
for i = 1:length(trace_struct) 
    ...
    % same thing but for protein data
    protein_nans = isnan(protein);
    tn_conv = conv(kernel, protein_nans);
    tn_conv = tn_conv(3:end-2);
    z_ids = find(tn_conv==5);
    z_ids = unique([z_ids-1 z_ids z_ids+1]);
    protein(z_ids) = 0;
    protein(protein < 0) = 0;
    ...
    if length(trace3) ~= sum(isnan(trace3))
    ...
    query_pointsp = time(isnan(protein));
    interp_tp = time(~isnan(protein));
    interp_fp = protein(~isnan(protein));
    new_fp = interp1(interp_tp, interp_fp, query_pointsp);
    protein(ismember(time, query_pointsp)) = new_fp;
    ...
    protein_interp = interp1(time, protein, time_interp);
    ...
    temp.protein_interp = protein_interp;
